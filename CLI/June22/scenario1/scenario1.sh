...
echo "Found db2 subnet with id $db2_subnet_id"
...
echo "Ec2 instance created with id $web_ec2_id"
username="ubuntu"
public_ip=$(aws ec2 describe-instances \
    --instance-ids $web_ec2_id \
    --query "Reservations[].Instances[].PublicIpAddress|[0]" \
    --output "text")
echo "Connect to the ec2 instance using ssh -i $keyname.pem $username@$public_ip"